/*
 * @lc app=leetcode.cn id=1770 lang=csharp
 *
 * [1770] 执行乘法运算的最大分数
 */
...
public class Solution {
    ...
    public int MaximumScore(int[] nums, int[] multipliers) 
    {
        ...
        var f = new int[n+1][];
        //f[i][j]的含义：multipliers数组取到的数索引为j，此时nums从头开始取到索引为i-1, i = 0说明只取了nums后面的数
        //根据数字关系 i <= j+1;
        for (int i = 0; i <= n; i++)
        {
            f[i] = new int[m];
        }
        f[0][0] = nums[n-1]*multipliers[0];
        ...
        for (int j = 1; j < m; j++)
        {
            for (int i = 0; i <= j+1; i++)
            {
                ...
                if (i == j+1)
                {
                    f[i][j] = f[i-1][j-1] + nums[i-1]*multipliers[j];
                    continue;
                }
                if (i == 0)
                {
                    f[i][j] = f[i][j-1] + nums[nums.Length - j + i-1] * multipliers[j];
                    continue;
                }
                f[i][j] = Math.Max(f[i-1][j-1] + nums[i-1]*multipliers[j], f[i][j-1] + nums[nums.Length - j + i-1] * multipliers[j]);
            }
        }
        var ans = int.MinValue;
        for (int i = 0; i <= m; i++)
        {
            ans = Math.Max(f[i][m-1], ans);
        }
        return ans;
    }
}
