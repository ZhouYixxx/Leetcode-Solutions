...
public class Solution403 {
    ...
    public bool CanCross1(int[] stones) 
    {
        ...
        for (int i = 1; i < n; i++)
        {
            for (int j = i+1; j < n; j++)           
            {
                var step = stones[j] - stones[i];
                var steps = step == 1 ? new int[]{1,2} : new int[]{step-1, step, step+1};
                //判断是否能找到dp[t][i] = true，满足t到i之间的距离为k-1,k,k+1
                var canCross = FindPre(stones, dp, i, steps);
                dp[i][j] = canCross;
            }
        }
        for (int i = 0; i < n-1; i++)
    ...
    //状态转移方程更合理的动态规划方法，复杂度O(n²)
    public bool CanCross(int[] stones)
    {
        var n = stones.Length;
        //dp[i][k] = true, 表示从某位置j跳到当前位置i是可行的，且步数为k, i > j 且 i >= k, j需要遍历[0...i]来寻找
        var dp = new bool[n][];
        for (int i = 0; i < n; i++)
        {
            dp[i] = new bool[n+1];
    ...
    //普通遍历的方式寻找Pre
    private bool FindPre(int[] stones, bool[][] dp, int curIndex, int[] steps)
    {
        int index = curIndex -1;
        while (index >= 0)
        {
            var step = stones[curIndex] - stones[index];
            if (!dp[index][curIndex])
            {
                index--;
                continue;
            }
            foreach (var stepOp in steps)
            {
                if (stepOp <= 0)
                    continue;
                if (step == stepOp)
    ...
    //借用二分查找的方式寻找Pre,似乎效果不大
    private bool FindPre1(int[] stones, bool[][] dp, int curIndex, int[] steps)
    {
        var l = 0;
        var r = curIndex - 1;
        int minK = steps[0];
        int maxK = steps.Length == 2 ? steps[1] : steps[2];
        //stones数组是个单调递增的数组，可以考虑使用二分辅助加快速度
        while (l < r)
        {
            var mid = (l + r)/2;
            var tempStep = stones[curIndex] - stones[mid];
            if (tempStep > maxK)
            {
                l = mid+1;
                continue;
        ...
        for (int i = l; i <= r; i++)
        {
            if (!dp[i][curIndex])
                continue;
            var tempStep = stones[curIndex] - stones[i];
